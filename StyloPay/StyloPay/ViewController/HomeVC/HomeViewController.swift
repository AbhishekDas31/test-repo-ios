...
class HomeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.feeLabel.text = "Exchange Fee:"
...
extension HomeViewController{
    func getExchangeRate(amount : Double){
        ...
        let url =  "\(mainUrl)/api/v1/exchangeRate?sourceCurrencyCode=\(fromCurrencyCode)&sourceAmount=&destinationCurrencyCode=\(toCurrencyCode)&destinationAmount=\(amount)"
            ...
            if error == nil {
                if let response = responseObject{
                    if let error =  response["error"] as? String {
                        ...
                        if let exchangeRate = response["exchangeRate"] as? Double{
                            self.feeLabel.text = "Exchange Fee:  \(exchangeRate)"
                        }
